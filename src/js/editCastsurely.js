...
window.onload = function () {
  ...
  var token = getToken();
  if(queryString.balance!=''){
    console.log(queryString)
    $('#balance-input').val(queryString.balance)
    queryEstimateCost()
  }
  ...
  $('.password-popup-buttons .confirm').on('tap', function () {
    ...
    $.ajax({
      ...
      data: {
        ...
        fixModel:1,
        expiryDate:queryString.expiryDate,
        scheduledProtocolId:queryString.scheduledProtocolId,
        checkRisk:false,
        paymentType:queryString.capitalMode
      },
      dataType: 'json',
      headers: token,
      type: 'post',
